...
void Session::handle_session(Logger &logger, net::yield_context yield) {
    ...
    while (true) {
        ...
        auto context = std::make_shared<Context>(
            remote_endpoint, target, req.method(),
            stream_.socket().get_executor(), std::ref(yield), logger);
...
bool Session::write_string_response(Response resp, net::yield_context &yield,
                                    beast::error_code &ec) {
...
bool Session::write_file_response(Response resp, net::yield_context &yield,
                                  beast::error_code &ec) {
